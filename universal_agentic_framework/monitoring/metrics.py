...
GRAPH_REQUESTS_TOTAL = Counter(
    ...
    ["profile_name", "status"],  # status: success, error
)
...
LLM_CALLS_TOTAL = Counter(
    ...
    "Total LLM API calls",
    ["profile_name", "provider", "model", "status"],
)
...
MEMORY_OPERATIONS_TOTAL = Counter(
    ...
    ["profile_name", "operation", "status"],  # operation: load, update, query
)
...
MEMORY_RETRIEVAL_SIGNAL_TOTAL = Counter(
    ...
    ["profile_name", "rated", "rating_bucket"],  # rated: yes/no; rating_bucket: none/low/mid/high
)
...
MESSAGE_FEEDBACK_TOTAL = Counter(
    ...
    ["profile_name", "value"],  # value: up, down, removed
)
...
CACHE_HITS_TOTAL = Counter(
    ...
    ["cache_type", "profile_name"],  # cache_type: llm, memory, crew, semantic
)

CACHE_MISSES_TOTAL = Counter(
    "cache_misses_total",
    "Total cache misses across all cache types",
    ["cache_type", "profile_name"],
)

CACHE_ERRORS_TOTAL = Counter(
    "cache_errors_total",
    "Total cache operation errors",
    ["cache_type", "profile_name", "operation"],  # operation: get, set, delete
)
...
CACHE_OPERATION_DURATION = Histogram(
    ...
    ["cache_type", "profile_name", "operation"],  # operation: get, set
    buckets=(0.001, 0.005, 0.01, 0.025, 0.05, 0.1, 0.25, 0.5),
)
...
VECTOR_DB_FALLBACK_TOTAL = Counter(
    ...
    ["profile_name", "reason"],  # reason: error, unavailable, timeout
)
...
CREW_EXECUTION_DURATION = Histogram(
    ...
    ["profile_name", "crew_name", "status"],  # status: success, error, timeout
...
PROFILE_ID_MISMATCH_TOTAL = Counter(
    ...
    ["profile_name", "active_profile_id", "reported_profile_id"],
)
...
@contextmanager
def track_graph_request(profile_name: str) -> Generator[dict[str, Any], None, None]:
    ...
    try:
        ...
        GRAPH_REQUEST_DURATION.labels(profile_name=profile_name).observe(duration)
...
@contextmanager
def track_node_execution(profile_name: str, node: str) -> Generator[None, None, None]:
...
def track_tokens(profile_name: str, model: str, node: str, tokens: int) -> None:
    """Track token usage.
...
def track_workspace_created(profile_name: str) -> None:
    WORKSPACE_CREATED_TOTAL.labels(profile_name=profile_name).inc()


def track_workspace_write_allowed(profile_name: str) -> None:
    WORKSPACE_WRITE_ALLOWED_TOTAL.labels(profile_name=profile_name).inc()


def track_workspace_write_denied(profile_name: str) -> None:
    WORKSPACE_WRITE_DENIED_TOTAL.labels(profile_name=profile_name).inc()


def track_workspace_intent_denied(profile_name: str, operation: str, reason: str) -> None:
...
def track_workspace_revised_copy_created(profile_name: str) -> None:
    WORKSPACE_REVISED_COPY_CREATED_TOTAL.labels(profile_name=profile_name).inc()


def track_workspace_cleanup_deleted(profile_name: str, deleted_count: int) -> None:
    if deleted_count <= 0:
        return
    WORKSPACE_CLEANUP_DELETED_TOTAL.labels(profile_name=profile_name).inc(deleted_count)
...
def update_active_sessions(profile_name: str, count: int) -> None:
    ...
    """
    ACTIVE_SESSIONS.labels(profile_name=profile_name).set(count)
...
def track_memory_importance_ranking(profile_name: str, duration: float) -> None:
    ...
    """
    MEMORY_IMPORTANCE_RANKING_DURATION.labels(profile_name=profile_name).observe(duration)


def track_memory_graph_statistics(profile_name: str, nodes: int, edges: int) -> None:
    ...
    """
    MEMORY_CO_OCCURRENCE_GRAPH_NODES.labels(profile_name=profile_name).set(nodes)
    MEMORY_CO_OCCURRENCE_GRAPH_EDGES.labels(profile_name=profile_name).set(edges)


def track_memory_quality(profile_name: str, importance_score: float) -> None:
    ...
    """
    MEMORY_QUALITY_SCORE.labels(profile_name=profile_name).observe(importance_score)


def track_related_memories(profile_name: str, count: int) -> None:
    ...
    """
    MEMORY_RELATED_RETRIEVED.labels(profile_name=profile_name).observe(count)


def track_memory_age(profile_name: str, age_days: float) -> None:
    ...
    """
    MEMORY_AGE_DAYS.labels(profile_name=profile_name).observe(age_days)
...
def track_memory_retrieval_signal(profile_name: str, user_rating: Any) -> None:
...
def track_memory_rated_after_retrieval(profile_name: str) -> None:
    """Emit a feedback-loop signal: the user rated a memory that was recently retrieved.

    Args:
        profile_name: Active profile identifier.
    """
    MEMORY_RATED_AFTER_RETRIEVAL_TOTAL.labels(profile_name=profile_name).inc()


def track_message_feedback(profile_name: str, value: str) -> None:
    ...
    """
    MESSAGE_FEEDBACK_TOTAL.labels(profile_name=profile_name, value=value).inc()


# Cache tracking functions

def track_cache_hit(cache_type: str, profile_name: str = "default") -> None:
    """Track cache hit.
    ...
    Args:
        ...
        profile_name: Name of the profile
    """
    CACHE_HITS_TOTAL.labels(cache_type=cache_type, profile_name=profile_name).inc()


def track_cache_miss(cache_type: str, profile_name: str = "default") -> None:
    """Track cache miss.
    ...
    Args:
        ...
        profile_name: Name of the profile
    """
    CACHE_MISSES_TOTAL.labels(cache_type=cache_type, profile_name=profile_name).inc()
...
def update_cache_hit_rate(
    ...
    Args:
        ...
        profile_name: Name of the profile
    """
    CACHE_HIT_RATE.labels(cache_type=cache_type, profile_name=profile_name).set(hit_rate)
...
def track_embedding_generation(profile_name: str, duration: float) -> None:
    ...
    Args:
        ...
        duration: Duration in seconds
    """
    EMBEDDING_GENERATION_DURATION.labels(profile_name=profile_name).observe(duration)


def update_embedding_cache_size(profile_name: str, size: int) -> None:
    ...
    """
    EMBEDDING_CACHE_SIZE.labels(profile_name=profile_name).set(size)


def track_semantic_similarity_match(crew_name: str, profile_name: str = "default") -> None:
...
def update_cache_size(
    ...
    Args:
        ...
        profile_name: Name of the profile
    """
    CACHE_SIZE_BYTES.labels(cache_type=cache_type, profile_name=profile_name).set(size_bytes)
...
def track_vector_db_search(profile_name: str, duration: float) -> None:
    ...
    """
    VECTOR_DB_SEARCH_DURATION.labels(profile_name=profile_name).observe(duration)
...
def track_vector_db_cleanup(profile_name: str, deleted_count: int) -> None:
    ...
    """
    VECTOR_DB_CLEANUP_TOTAL.labels(profile_name=profile_name).inc(deleted_count)


def track_vector_db_fallback(profile_name: str, reason: str) -> None:
    ...
    """
    VECTOR_DB_FALLBACK_TOTAL.labels(profile_name=profile_name, reason=reason).inc()
...
def track_crew_execution(
    ...
    CREW_EXECUTION_DURATION.labels(
        profile_name=profile_name, crew_name=crew_name, status=status
    ).observe(duration)
...
def track_crew_retry(profile_name: str, crew_name: str) -> None:
    """Increment crew retry counter."""
    CREW_RETRIES_TOTAL.labels(profile_name=profile_name, crew_name=crew_name).inc()


def track_crew_timeout(profile_name: str, crew_name: str) -> None:
    """Increment crew timeout counter."""
    CREW_TIMEOUTS_TOTAL.labels(profile_name=profile_name, crew_name=crew_name).inc()


@contextmanager
def track_crew_chain(profile_name: str, chain_name: str) -> Generator[None, None, None]:
    ...
    try:
        ...
        CREW_CHAIN_EXECUTION_DURATION.labels(
            profile_name=profile_name, chain_name=chain_name
        ).observe(duration)


@contextmanager
def track_crew_parallel(profile_name: str) -> Generator[None, None, None]:
    ...
    try:
        ...
        duration = time.time() - start_time
        CREW_PARALLEL_EXECUTION_DURATION.labels(profile_name=profile_name).observe(duration)


def track_crew_validation_failure(profile_name: str, crew_name: str) -> None:
    """Track crew result validation failure."""
    CREW_VALIDATION_FAILURES.labels(profile_name=profile_name, crew_name=crew_name).inc()
