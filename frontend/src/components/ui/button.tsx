...
function Button({
  ...
  const mappedSize = sizeMap[size as string] ?? size

  const cvaOpts = {
    variant: mappedVariant ?? "default",
    size: mappedSize ?? "default",
    className,
  } as Parameters<typeof buttonVariants>[0]

  if (asChild) {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { render: _render, focusableWhenDisabled: _focus, ...slotProps } = props
    return (
      <Slot
        className={cn(buttonVariants(cvaOpts))}
