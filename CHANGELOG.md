...
- **feat** Phase-3 shell extraction: added `components/product/AppShell.tsx` and migrated `LayoutShell.tsx` main app chrome container to the shared shell primitive.
- **feat** Phase-3 shell extraction: added `components/product/ChatMessageShell.tsx` and migrated assistant/user/queued/streaming message row shells in `ChatInterface.tsx` to the shared chat-message shell.
- **feat** Phase-3 shell extraction: added `components/workspace/WorkspacePanelShell.tsx` and `WorkspacePanelTopBar.tsx`, then migrated `workspace/WorkspacePanel.tsx` outer panel chrome and top header to shared workspace-shell primitives.
- **fix** Route surface parity: replaced hardcoded white page backgrounds in `app/settings/Settings.module.css` and `app/metrics/Metrics.module.css` with semantic theme tokens so `/settings`, `/metrics`, `/admin`, and `/admin/rag` now respect light/dark mode page surfaces.
- **feat** Added `components/product/PageShell.tsx` and `PageHeader.tsx` to standardize non-chat page container/background spacing and heading composition.
- **feat** Migrated `app/settings/page.tsx`, `app/admin/page.tsx`, `app/admin/rag/page.tsx`, and `app/metrics/page.tsx` to shared `PageShell`/`PageHeader` primitives.
- **feat** Migrated `app/chats/page.tsx` and `app/memories/page.tsx` to the shared `PageShell` primitive to remove duplicated route-level main wrappers and keep page-surface behavior consistent.
