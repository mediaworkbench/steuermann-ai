...
The heartbeat is a *virtual cron* — the foundation for proactive agent behavior. A single global "beat" fires every *N* minutes; each beat **enqueues** work — once per `global` task, once per active user for `per_user` tasks — and a worker pool runs each through a four-phase tick: **observe → reason → act → log**. Today the phases are no-op hooks (no external API calls or agent invocation); the scheduling, fan-out, persistence, and observability scaffolding is in place so future work plugs into the phases directly.
...
- `heartbeat_beat` — `IntervalTrigger(minutes=rate)`, `max_instances=1` (a slow enqueue never overlaps the next beat). A non-blocking enqueuer: `global` tasks push one `(task, None)` item; `per_user` tasks push one `(task, user_id)` per active user (`UserStore.get_active_user_ids()`). A `QUEUE_HIGH_WATER` check skips the beat's enqueue when the previous beat is still draining.
- `heartbeat_control` — a fixed 30s poll that reads the admin-set rate from Postgres (written by FastAPI, a separate process) and reschedules the beat **only when the rate actually changed** (rescheduling resets `next_run_time`, so an unconditional reschedule would prevent the beat from ever firing), and periodically prunes `heartbeat_runs` to a retention window.

**Worker pool + queue.** The fan-out queue is an unbounded in-process `asyncio.Queue` drained by `HEARTBEAT_WORKERS` worker coroutines (spawned in `start()`, cancelled in `stop()`). The worker count is the throttle, so a large user set never blocks the beat or floods the LLM. A worker never dies on a tick error. The queue is **not durable**: on a restart mid-drain the next beat re-enqueues, and per-user cooldown prevents doubles.
